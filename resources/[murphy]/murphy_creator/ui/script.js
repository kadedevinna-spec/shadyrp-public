...
function hideCharSelectMenu() {
    ...
    const fallbackPanel = document.getElementById('charSelectFallbackPanel');
    if (fallbackPanel) {
...
function createPinSelection(pinPerso) {
    ...
    pinsContainer.style.position = 'fixed';
    pinsContainer.style.left = '4vw';
    pinsContainer.style.top = '18vh';
    pinsContainer.style.width = '26vw';
    pinsContainer.style.zIndex = '9999';

    const oldFallbackPanel = document.getElementById('charSelectFallbackPanel');
    if (oldFallbackPanel) {
        oldFallbackPanel.remove();
    }

    const fallbackPanel = document.createElement('div');
    fallbackPanel.id = 'charSelectFallbackPanel';
    fallbackPanel.style.position = 'fixed';
    fallbackPanel.style.left = '2vw';
    fallbackPanel.style.top = '10vh';
    fallbackPanel.style.width = '360px';
    fallbackPanel.style.maxHeight = '78vh';
    fallbackPanel.style.overflowY = 'auto';
    fallbackPanel.style.background = 'rgba(10, 10, 10, 0.86)';
    fallbackPanel.style.border = '2px solid rgba(172, 11, 16, 0.9)';
    fallbackPanel.style.borderRadius = '10px';
    fallbackPanel.style.padding = '14px';
    fallbackPanel.style.zIndex = '10000';
    fallbackPanel.style.boxShadow = '0 10px 30px rgba(0,0,0,0.45)';
    fallbackPanel.style.color = '#fff';
    fallbackPanel.style.fontFamily = 'Hapna, serif';

    const panelTitle = document.createElement('div');
    panelTitle.textContent = 'Character Select';
    panelTitle.style.fontFamily = 'RDRLino, serif';
    panelTitle.style.fontSize = '28px';
    panelTitle.style.marginBottom = '10px';
    fallbackPanel.appendChild(panelTitle);

    const panelSub = document.createElement('div');
    panelSub.textContent = pinPerso.length > 0 ? 'Choose a character to continue.' : 'No character slots were received.';
    panelSub.style.fontSize = '14px';
    panelSub.style.opacity = '0.9';
    panelSub.style.marginBottom = '12px';
    fallbackPanel.appendChild(panelSub);

    const panelList = document.createElement('div');
    panelList.id = 'charSelectFallbackList';
    fallbackPanel.appendChild(panelList);

    document.body.appendChild(fallbackPanel);

    pinPerso.forEach((pin, index) => {
        const fixedTop = `${index * 84}px`;

        if (pin.ExistingChar === true) {
            const pinElem = document.createElement('div');
            pinElem.classList.add('container_select');
            pinElem.id = 'pinSelect' + pin.id;
            pinElem.style.position = 'relative';
            pinElem.style.left = '0';
            pinElem.style.top = fixedTop;
            pinElem.style.marginBottom = '12px';
            pinElem.style.width = '100%';
            pinElem.style.zIndex = '9999';
        ...
        `;
            ...
            pinsContainer.appendChild(pinElem);

            const headElem = pinElem.querySelector('.head-indicator-select');
            if (headElem) {
                headElem.style.position = 'relative';
                headElem.style.top = '0';
                headElem.style.left = '0';
                headElem.style.transform = 'none';
                headElem.style.width = '100%';
                headElem.style.maxWidth = '340px';
            }
            ...
            fallbackButton.textContent = pin.name;
            fallbackButton.style.display = 'block';
            fallbackButton.style.width = '100%';
            fallbackButton.style.marginBottom = '10px';
            fallbackButton.style.padding = '14px 16px';
            fallbackButton.style.background = 'rgba(36,36,36,0.95)';
            fallbackButton.style.color = '#fff';
            fallbackButton.style.border = '1px solid rgba(255,255,255,0.14)';
            fallbackButton.style.borderRadius = '8px';
            fallbackButton.style.textAlign = 'left';
            fallbackButton.style.fontFamily = 'Hapna, serif';
            ...
            panelList.appendChild(fallbackButton);
        ...
        `;
            ...
            const fallbackButton = document.createElement('button');
            fallbackButton.type = 'button';
            fallbackButton.textContent = t('charSelect.createNew');
            fallbackButton.style.display = 'block';
            fallbackButton.style.width = '100%';
            fallbackButton.style.marginBottom = '10px';
            fallbackButton.style.padding = '14px 16px';
            fallbackButton.style.background = 'rgba(117,17,41,0.95)';
            fallbackButton.style.color = '#fff';
            fallbackButton.style.border = '1px solid rgba(255,255,255,0.14)';
            fallbackButton.style.borderRadius = '8px';
            fallbackButton.style.textAlign = 'left';
