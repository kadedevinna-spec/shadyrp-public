local JobData = {
    name = filename(),
    label = "Random Item Delivery",
    description = "Deliver items to random locations",
    steps = {
        { action = "DeliverItem" },
        { action = "DeliverItem" },
        { action = "DeliverItem" },
...
if IsDuplicityVersion() then
    ...
    local deliveryLocations = {
        { label = "Deliver the item", description = "Deliver the item to the specified location",  coords = vector3(-3312.757, -2826.725, -6.813381), npc = { "a_m_m_armtownfolk_01", 90.0 },  item = { "water", 1 } },
        ...
        { label = "Deliver the item", description = "Deliver the item to the specified location",  coords = vector3(-5217.904, -2120.502, 11.68457),  npc = { "a_m_m_armtownfolk_01", 0.0 },   item = { "water", 1 } },
        { label = "Deliver the item", description = "Deliver the item to the specified location",  coords = vector3(-5498.94, -2932.629, -1.423192),  npc = { "a_m_m_armtownfolk_01", 45.0 },  item = { "water", 1 } },
        { label = "Deliver the item", description = "Deliver the item to the specified location",  coords = vector3(-5987.83, -3229.936, -22.14742),  npc = { "a_m_m_armtownfolk_01", 135.0 }, item = { "water", 1 } },
        { label = "Deliver the item", description = "Deliver the item to the specified location",  coords = vector3(-5225.704, -3480.168, -21.56653), npc = { "a_m_m_armtownfolk_01", 225.0 }, item = { "water", 1 } },
    }
    ...
    actions["DeliverItem"] = function(job)
        ...
        local pedblip = CreateBlipEntity(npcEntity, job.steps[job.currentStep].label, `BLIP_STYLE_OBJECTIVE`, nil,
            `blip_ambient_vip`)
                            ...
                            if amount >= neededamount then
                                Callback.triggerServer('RemoveItem', function(result)
                                    if result then
                                        ...
                                        exports.murphy_interact:RemoveEntityInteraction(
                                        NetworkGetNetworkIdFromEntity(npcEntity), job.name .. "_step" .. job.currentStep)
                                        job:nextStep()
            ...
            while true do
                ...
                if step ~= job.currentStep or job.abandon == true then
                    ...
                    if job.abandon == true then
                        exports.murphy_interact:RemoveEntityInteraction(NetworkGetNetworkIdFromEntity(npcEntity),
                            job.name .. "_step" .. job.currentStep)
                        DeleteEntity(npcEntity)
                        Utils.StopJob()
                    end
                    break
    ...
    function Utils.createJob(jobData)
        ...
        for _, step in ipairs(jobData.steps) do
            if step.action == "DeliverItem" then
                local location
                repeat
                    location = deliveryLocations[math.random(#deliveryLocations)]
