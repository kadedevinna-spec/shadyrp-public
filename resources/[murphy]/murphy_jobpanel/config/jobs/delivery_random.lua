...
local JobData = {
    ...
    label = "Random Delivery",
    description = "Deliver packages to random locations",
    steps = {
        {
        ...
        { action = "DeliverPackage", label = "Deliver the package" }, --action = function to execute, label = text to display on active job menu
    ...
    board = { "armadillo" },              -- Boards where the job is available
    image = "./img/Poster_art/bread.png", -- Path to the job poster image
    reward = "$25",                       -- Text Reward on the poster
}
if IsDuplicityVersion() then
    ...
    local deliveries = {
        { Name = "Church",           Pos = vector3(-3312.757, -2826.725, -6.813381), Description = "Deliver the package to the Church" },
        ...
        { Name = "Gaptooth Breach",  Pos = vector3(-5987.83, -3229.936, -22.14742),  Description = "Deliver the package to Gaptooth Breach" },
        { Name = "Benedict Point",   Pos = vector3(-5225.704, -3480.168, -21.56653), Description = "Deliver the package to Benedict Point" },
    }
    ...
    local deliverLocation = 1
    local packagetodeliver
    local blips = {}
    ...
    function Utils.NewLocation(job)
        ...
        deliverLocation = rnd(1, #deliveries)
        while tableContains(deliverypoints, deliverLocation) do
            deliverLocation = rnd(1, #deliveries)
        end
    ...
    function Utils.createJob(jobData) --- Create the job
        ...
        for _, step in ipairs(jobData.steps) do
            if step.action == "DeliverPackage" then
                local location
                repeat
                    location = deliveries[math.random(#deliveries)]
                ...
                packagetodeliver = packagetodeliver + 1
            end
