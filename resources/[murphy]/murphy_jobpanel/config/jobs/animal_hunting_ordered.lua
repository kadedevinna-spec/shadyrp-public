local JobData = {
    ...
    description = "Hunt animals in a specific order",
    steps = {
        { action = "HuntAnimal" },
        { action = "HuntAnimal" },
    ...
    board = { "armadillo" },
    image = "./img/Poster_art/hunt1.png",
    reward = "$100",
}
...
if IsDuplicityVersion() then
    ...
    local huntingLocations = {
        { label = "Hunt wolves near the lake", description = "Hunt a pack of wolves near the lake in Armadillo", coords = vector3(-3349.514, -3127.722, -5.881188), radius = 50.0, animal = "a_c_wolf", count = 3 },
        { label = "Hunt cougars near the farm", description = "Hunt cougars threatening a farm", coords = vector3(-4853.64, -2557.071, -5.135025), radius = 50.0, animal = "a_c_cougar_01", count = 2 },
    }
            ...
            while true do
                ...
                if #(playerCoords - coords) < 200.0 then
                    for _, animal in ipairs(animals) do
                        ...
                        local animalEntity = CreatePed(GetHashKey(animal), randomCoords.x, randomCoords.y, randomCoords
                            .z, 0.0, true, true)
    ...
    function Utils.createJob(jobData)
        for i, step in ipairs(jobData.steps) do
            if step.action == "HuntAnimal" then
                local location = huntingLocations[i]
