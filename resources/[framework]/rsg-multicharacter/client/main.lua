...
local function hasValidAppearance(appearanceData)
    return type(appearanceData) == "table" and type(appearanceData.skin) == "table"
end
...
local function startNewCharacterCreation(data)
    selectingChar = false
    DoScreenFadeOut(150)
    Wait(200)
    openCharMenu(false)
...
local function loadExistingCharacter(cData)
    RSGCore.Functions.TriggerCallback('rsg-multicharacter:server:getAppearance', function(appearanceData)
        selectingChar = false
        DoScreenFadeOut(10)
        openCharMenu(false)
        cleanPed(charPed)
        if hasValidAppearance(appearanceData) then
            ...
            TriggerServerEvent('rsg-multicharacter:server:loadUserData', cData, 'newspawn')
        end
    end, cData.citizenid)
end
...
RegisterNetEvent('rsg-multicharacter:client:chooseChar', function()
    ...
    ShutdownLoadingScreenNui()
    Wait(10)
    RSGCore.Functions.TriggerCallback("rsg-multicharacter:server:setupCharacters", function(characters)
        local hasCharacters = type(characters) == "table" and #characters > 0
        if not hasCharacters then
            startNewCharacterCreation({ cid = "1", emergencyDirect = true })
            return
        end

        loadExistingCharacter(characters[1])
    end)
end)
...
RegisterNUICallback('createNewCharacter', function(data, cb) -- Creating a char
    startNewCharacterCreation(data)
    Wait(500)
    cb('ok')
end)
