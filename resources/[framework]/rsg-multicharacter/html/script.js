...
function ensureFallbackPanel() {
    var existing = document.getElementById('multichar-fallback-panel');
    if (existing) return existing;

    var panel = document.createElement('div');
    panel.id = 'multichar-fallback-panel';
    ...
    panel.style.background = 'rgba(10,10,10,0.90)';
    panel.style.border = '2px solid rgba(172, 11, 16, 0.95)';
    ...
    panel.style.fontFamily = "'RDR Lino Regular', sans-serif";

    var title = document.createElement('div');
    title.id = 'multichar-fallback-title';
    ...
    help.id = 'multichar-fallback-help';
    help.textContent = 'If the normal menu does not show, use these buttons.';
    ...
    list.id = 'multichar-fallback-list';
...
function showFallbackPanel() {
    ensureFallbackPanel().style.display = 'block';
}

function hideFallbackPanel() {
    var panel = document.getElementById('multichar-fallback-panel');
    if (panel) panel.style.display = 'none';
}

function renderFallbackButtons() {
    var panel = ensureFallbackPanel();
    var list = document.getElementById('multichar-fallback-list');
    if (!list) return;
    list.innerHTML = '';

    for (let i = 1; i <= (Number(NChar) || 5); i++) {
        var slot = document.getElementById('char-' + i);
        ...
        button.style.background = 'rgba(36,36,36,0.95)';
        button.style.color = '#fff';
        button.style.border = '1px solid rgba(255,255,255,0.14)';
        button.style.borderRadius = '8px';
        button.style.textAlign = 'left';
        button.style.fontFamily = "'RDR Lino Regular', sans-serif";
        button.style.fontSize = '18px';
        button.style.cursor = 'pointer';

        var label = 'Slot ' + i + ' - Empty Slot';
        var isEmpty = true;
        if (slot) {
            var cid = $(slot).data('cid');
            var cData = $(slot).data('cData');
            if (cid !== undefined && cid !== '') {
                isEmpty = false;
                label = 'Slot ' + i + ' - ' + (cData && cData.charinfo ? (cData.charinfo.firstname + ' ' + cData.charinfo.lastname) : 'Character');
            }
        }
        if (isEmpty) {
            label = 'Create Character in Slot ' + i;
            button.style.background = 'rgba(117,17,41,0.95)';
        }
...
$(document).ready(function (){
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "setupCharacters") {
            setupCharacters(Array.isArray(event.data.characters) ? event.data.characters : [])
        }
