...
Birthdate = nil
Cid = nil
EmergencyDirectCreate = false
EmergencyDirectSkinOnly = false
...
function ApplySkin()
    ...
    RSGCore.Functions.TriggerCallback('rsg-multicharacter:server:getAppearance', function(data)
        if type(data) ~= 'table' or type(data.skin) ~= 'table' then
            if _Target == PlayerPedId() then
                SetEntityAlpha(_Target, 255)
                SetEntityVisible(_Target, true)
            end
            promise:resolve()
            return
        end
...
RegisterNetEvent('rsg-appearance:client:OpenCreator', function(data, empty)
    if data then
        Cid = data.cid
        EmergencyDirectCreate = data.emergencyDirect == true
        EmergencyDirectSkinOnly = false
    elseif empty then
        Skinkosong = true
        EmergencyDirectCreate = false
        EmergencyDirectSkinOnly = true
    end
...
function EmergencyDirectCreateFlow()
    local slotSuffix = tonumber(Cid) or 1
    Firstname = 'Shady'
    Lastname = 'Traveler' .. tostring(slotSuffix)
    ...
    lib.notify({
        ...
        description = 'Creating a temporary starter character so you can get in-game.',
        type = 'inform',
        duration = 5000
    })
...
function EmergencyDirectSkinOnlyFlow()
    LoadedComponents = CreatorCache

    lib.notify({
        title = 'Character Recovery',
        description = 'Finishing character setup and sending you into the world.',
        type = 'inform',
        duration = 5000
    })
