...
function SpawnPeds()
    ...
    if EmergencyDirectCreate then
        StartCharacterCreatorCamera(1, cameraMale)
        return
    end
...
function StartCharacterCreatorCamera(selected, camera)
    ...
    IsInCharCreation = true
    if EmergencyDirectCreate or EmergencyDirectSkinOnly then
        CreateThread(function()
            Wait(250)
            if EmergencyDirectCreate then
                EmergencyDirectCreateFlow()
            else
                EmergencyDirectSkinOnlyFlow()
            end
        end)
        return
    end
