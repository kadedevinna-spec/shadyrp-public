...
RSG.Zones1 = type(RSG.Zones1) == "table" and RSG.Zones1 or {}
RSG.Cloakroom = type(RSG.Cloakroom) == "table" and RSG.Cloakroom or {}
RSG.CreatedEntries = type(RSG.CreatedEntries) == "table" and RSG.CreatedEntries or {}

local function ExitClothingStore()
    if next(CurentCoords) == nil and RSG.Zones1[1] then
        CurentCoords = RSG.Zones1[1]
    end

    if CurentCoords and CurentCoords.quitcoords then
...
CreateBlips = function()
    for _, coordsList in pairs(RSG.Zones1) do
        if coordsList.blipcoords and #coordsList.blipcoords > 0 and coordsList.showblip then
