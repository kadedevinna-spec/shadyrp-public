-- If a native DataView is already provided by the runtime, don't override it
if type(DataView) == 'table' and type(DataView.ArrayBuffer) == 'function' then
    return
end

local _strblob = string.blob or function(length)
    print('[GM][warning] string.blob is not available; DataView buffers may be immutable and some natives may not write results (expect zeros).')
