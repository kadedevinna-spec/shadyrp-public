...
-- Resolve a ped model hash to its model name using the `Peds` list once per resource lifetime
local _pedHashToName = nil
local function ensurePedHashMap()
	if _pedHashToName ~= nil then return end
	_pedHashToName = {}
	for _, name in ipairs(Peds) do
		local h = GetHashKey and GetHashKey(name)
		if h then _pedHashToName[h] = name end
	end
end


function ResolvePedModelName(modelHash)
	--Bridge.Debug('Resolving ped model name for hash: ' .. modelHash)
	ensurePedHashMap()
	local n = _pedHashToName and _pedHashToName[modelHash]
	if n and n ~= '' then return n end
	-- Fallback: hex string of hash
	local asNum = tonumber(modelHash) or 0
	return string.format('0x%X', asNum)
end
